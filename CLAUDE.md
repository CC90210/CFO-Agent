...
1. **READ `brain/CAPABILITIES.md`** — auto-generated registry of every skill, tool, command, and module Atlas has. This is the routing map: given a user question, this file tells you which capability handles it. Regenerated by `python scripts/refresh.py`.
...
4. **READ auto-memory** in `C:\Users\User\.claude\projects\c--Users-User-APPS-CFO-Agent\memory\` — start with `MEMORY.md` (auto-regenerated index), then pull specific files by topic. Especially `user_financial_profile.md`, `project_2025_tax_return_filed.md`, `feedback_no_redundant_questions.md`, `feedback_telegram_brevity.md`.
5. **READ Bravo's pulse** if it exists — `C:\Users\User\Business-Empire-Agent\data\pulse\ceo_pulse.json`
6. **NEVER assume or guess** CC's financial situation. Data is in files.
7. **NEVER suggest CC do something he's already done** (FHSA opened, 2025 return filed, CRA identity blocked, etc.).
8. **NEVER ask for info that's documented.** Check USER.md + memory FIRST.
